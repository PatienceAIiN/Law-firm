      ...
      <div className="mx-auto max-w-6xl rounded-[28px] border border-slate-200 bg-gradient-to-b from-white via-[#fbfcfe] to-white p-5 shadow-sm sm:p-8">
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-stretch">
          <div className="space-y-5">
            ...
            <h2 className="text-3xl font-black uppercase tracking-tighter text-[var(--primary)] sm:text-5xl">
              {section.heading?.split(' ').slice(0, 2).join(' ') || 'Guided by'} <span className="text-[var(--primary)]">{section.heading?.split(' ').slice(2).join(' ') || 'Core Principles'}</span>
            </h2>
              ...
              {highlights.map((value: any, index: number) => {
                ...
                return (
                  <div key={value.title} className="rounded-[22px] border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-white p-4 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
                    <div className="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-[var(--primary)]/5 text-[var(--primary)]">
                      <Icon className="h-5 w-5" />
                    </div>
                    <h3 className="text-sm font-black uppercase tracking-[0.2em] text-[var(--primary)]">{value.title}</h3>
          ...
          <div className="rounded-[26px] border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.05)]">
            <div className="mb-4 inline-flex items-center rounded-full border border-[#F4E8D8]/20 bg-[#F6F0E8]/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-[var(--primary)]">
            ...
            <div className="mt-6 grid gap-3">
              {(section.missionPoints || []).map((item: any) => (
                <div key={item.title} className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  <div className="text-sm font-black uppercase tracking-[0.18em] text-[var(--primary)]">{item.title}</div>
                  <div className="mt-1 text-xs font-medium text-slate-500">{item.subtitle}</div>
                </div>
              ))}
            </div>
            <div className="mt-6">
              <Link href="/about" className="inline-flex items-center gap-2 rounded-2xl border border-[var(--primary)] bg-[var(--primary)] px-5 py-3 text-[11px] font-black uppercase tracking-widest text-white transition-all hover:bg-[#F6F0E8] hover:text-[var(--primary)]">
                Read More
              </Link>
            </div>
          </div>
