          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-700">
            <div className="space-y-4">
              ...
              <h2 className="text-3xl lg:text-5xl font-bold text-[#0a192f] leading-tight">
                {title} <span className="text-[#c5a059]">{name}</span>
              </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button asChild className="bg-[#0a192f] hover:bg-[#112240] text-white px-8 py-6 rounded-xl text-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                <Link href="/about" className="flex items-center gap-2">
                  {section.ctaPrimaryText || 'Learn More About Us'}
                  <ArrowRight className="w-4 h-4" />
                </Link>
              </Button>
              <Button asChild variant="outline" className="border-[#0a192f]/15 px-8 py-6 rounded-xl text-lg text-[#0a192f] transition-all duration-300 hover:-translate-y-1 hover:bg-[#0a192f] hover:text-white hover:shadow-lg">
          ...
          <div className="relative animate-in fade-in slide-in-from-right duration-700">
            <div className="rounded-[2rem] border border-gray-100 bg-gradient-to-br from-white via-slate-50 to-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] transform rotate-2">
              <div className="rounded-[1.5rem] bg-gradient-to-br from-white via-slate-50 to-white p-12 text-center -rotate-2">
                <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full border-8 border-white bg-gradient-to-br from-[#0a192f] to-[#112240] shadow-2xl">
                  <span className="text-white text-3xl font-bold">
                    {name.split(' ').map((n: string) => n[0]).join('')}
                  </span>
                </div>
                <h3 className="text-3xl font-bold text-[#0a192f] mb-2">{name}</h3>
                <p className="text-[#b8872f] font-medium text-lg mb-8">{title}</p>
                
                <div className="space-y-4 text-left">
                  {highlights.map((item: string, i: number) => (
                    <div key={i} className="flex items-center gap-3 text-slate-700">
                      <div className="w-5 h-5 bg-[#0a192f] rounded-full flex items-center justify-center flex-shrink-0">
            ...
            <div className="absolute -z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-[#c5a059]/15 rounded-full blur-[90px]"></div>
          </div>
        </div>
      </div>
    </section>
