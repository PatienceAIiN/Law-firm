        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-700">
            <div className="space-y-4">
              <div className="inline-flex items-center px-3 py-1 rounded-full bg-[#14203E]/5 border border-[#14203E]/10 text-[#14203E] text-sm font-medium">
                <span>{section.badge || 'Professional Profile'}</span>
              </div>
              
              <h2 className="text-3xl lg:text-5xl font-bold text-[#14203E] leading-tight">
                {title} <span className="text-[#14203E]">{name}</span>
              </h2>
                  ...
                  return (
                    <div key={index} className="text-center">
                      <div className="w-12 h-12 bg-[#F6F0E8]/10 rounded-xl flex items-center justify-center mx-auto mb-3">
                        <Icon className="w-6 h-6 text-[#14203E]" />
                      </div>
                      <div className="text-2xl font-bold text-[#14203E]">{stat.value}</div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button asChild className="bg-[#14203E] hover:bg-[#112240] text-white px-8 py-6 rounded-xl text-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                <Link href="/about" className="flex items-center gap-2">
                  {section.ctaPrimaryText || 'Learn More About Us'}
                  <ArrowRight className="w-4 h-4" />
                </Link>
              </Button>
              <Button asChild variant="outline" className="border-[#14203E]/15 px-8 py-6 rounded-xl text-lg text-[#14203E] transition-all duration-300 hover:-translate-y-1 hover:bg-[#14203E] hover:text-white hover:shadow-lg">
          ...
          <div className="relative animate-in fade-in slide-in-from-right duration-700">
            <div className="rounded-[2rem] border border-gray-100 bg-gradient-to-br from-white via-slate-50 to-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] transform rotate-2">
              <div className="rounded-[1.5rem] bg-gradient-to-br from-white via-slate-50 to-white p-12 text-center -rotate-2">
                ...
                <h3 className="text-3xl font-bold text-[#14203E] mb-2">{name}</h3>
                <p className="text-[#b8872f] font-medium text-lg mb-8">{title}</p>
                
                <div className="space-y-4 text-left">
                  {highlights.map((item: string, i: number) => (
                    <div key={i} className="flex items-center gap-3 text-slate-700">
                      <div className="w-5 h-5 bg-[#14203E] rounded-full flex items-center justify-center flex-shrink-0">
