    ...
    <section id="practice-areas" className="scroll-mt-28 px-3 py-4 sm:px-4 sm:py-6">
      <div className="mx-auto max-w-7xl rounded-[28px] border border-slate-200 bg-gradient-to-b from-white via-[#fbfcfe] to-white p-5 shadow-sm sm:p-8">
        <div className="text-center space-y-4 mb-12">
          <div className="inline-flex items-center px-3 py-1 rounded-full bg-[#0a192f]/5 border border-slate-200 text-[#0a192f] text-xs font-semibold uppercase tracking-[0.24em]">
            <span>{section.badge || 'Practice Areas'}</span>
          </div>

          <h2 className="text-2xl lg:text-4xl font-black text-[#0a192f] leading-tight uppercase tracking-tighter">
            {section.title?.split(' ')[0] || 'Browse'} <span className="text-[#c5a059]">{section.title?.split(' ').slice(1).join(' ') || 'Legal Services'}</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((area) => {
            ...
            return (
              ...
              >
                <div className="absolute top-0 right-0 h-32 w-32 -mr-16 -mt-16 rounded-full bg-[#c5a059]/10 transition-colors group-hover:bg-[#c5a059]/15" />

                <div className="relative z-10 space-y-5">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[#c5a059]/10 bg-[#c5a059]/10 text-[#c5a059] transition-transform duration-500 group-hover:scale-110">
                    <Icon className="h-7 w-7" />
                  </div>

                  <div>
                    <h3 className="text-2xl font-black uppercase tracking-tighter text-[#0a192f]">
                      {area.title}
                    </h3>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-2xl border border-[#0a192f] bg-[#0a192f] px-5 py-3 text-[11px] font-black uppercase tracking-widest text-white transition-all hover:bg-[#c5a059] hover:text-[#0a192f]"
                  >
                    Learn More
          ...
          <div className="relative z-10 w-full max-w-5xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-6">
              <div>
                ...
                <h3 className="mt-1 text-xl font-black uppercase tracking-tight text-[#0a192f] sm:text-2xl">
                  {selectedArea.title}
                </h3>
              </div>
              <button
                type="button"
                onClick={() => setSelectedArea(null)}
                className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-500 transition-colors hover:text-[#0a192f]"
                ...
                <div className="space-y-5">
                  ...
                  <div className="rounded-[28px] border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-white p-6">
                    ...
                    <button
                      ...
                      className="mt-5 inline-flex items-center gap-2 rounded-2xl border border-[#0a192f] bg-[#0a192f] px-5 py-3 text-[11px] font-black uppercase tracking-widest text-white transition-all hover:bg-[#c5a059] hover:text-[#0a192f]"
                    >
