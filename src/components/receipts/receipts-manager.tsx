  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-[#14203E] flex items-center gap-2"><FileText className="w-5 h-5 text-[#64748b]" /> Receipts</h2>
        <button onClick={() => { setForm(blankForm()); setShowForm((s) => !s); setError('') }} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#14203E] text-white text-sm font-semibold hover:bg-[#1d2c52]">
      ...
      {showForm && (
        <div className="rounded-2xl border border-[#F4E8D8] bg-[#FFFCF8] p-5 space-y-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 pt-2 border-t border-[#F4E8D8]">
            <div className="text-sm text-[#14203E]">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => save(false)} disabled={busy} className="px-4 py-2 rounded-xl border border-[#14203E] text-[#14203E] text-sm font-semibold disabled:opacity-60">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : (form.id ? 'Update' : 'Generate & Save')}</button>
              <button onClick={() => save(true)} disabled={busy || !form.clientEmail} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#14203E] text-white text-sm font-semibold disabled:opacity-60"><Send className="w-4 h-4" /> {form.id ? 'Update & Send' : 'Generate & Send'}</button>
            </div>
          </div>
        </div>
      )}
            ...
            {receipts.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-[#FFFCF8]">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-[#14203E]">{r.number}</span>
      ...
      {toast && <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[130] bg-[#14203E] text-white text-sm px-4 py-2 rounded-xl shadow-lg inline-flex items-center gap-2"><Check className="w-4 h-4" /> {toast}</div>}
    </div>
  )
}
