  ...
  return (
    <div className="bg-white rounded-[2.5rem] card-3d border border-gray-100 p-10">
      <div className="flex items-center justify-between mb-10">
        <h2 className="text-xl font-black text-primary uppercase tracking-tighter">Activity Overview</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {stats.map((stat, index) => (
          <Link key={index} href={stat.href} className="group">
            <div className="p-6 bg-[#f8fafc] border border-gray-100 rounded-3xl hover:bg-primary hover:shadow-xl transition-all duration-500">
              <div className="flex items-center justify-between mb-6">
                <div className="w-12 h-12 bg-[#F6F0E8]/10 rounded-2xl flex items-center justify-center group-hover:bg-[#F6F0E8] transition-colors">
                  <stat.icon className="w-6 h-6 text-primary group-hover:text-primary transition-colors" />
                </div>
              </div>
              
              <div className="space-y-1">
                <div className="text-4xl font-black text-primary group-hover:text-white transition-colors tracking-tighter">{stat.value}</div>
