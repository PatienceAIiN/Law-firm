    ...
    <div className="rounded-2xl border border-[#F4E8D8] bg-white overflow-hidden transition-shadow hover:shadow-md">
      <div className="p-4 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-primary text-sm truncate">{request.recipientName}</div>
      ...
      {request.status === 'SUBMITTED' && expanded && (
        <div className="mx-4 mb-4 rounded-xl border border-amber-100 bg-amber-50/60 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <div className="font-semibold text-primary text-sm">{request.submittedName}</div>
              {request.submittedRole && <div className="text-xs text-gray-500">{request.submittedRole}</div>}
            </div>
            {request.submittedRating && (
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-3.5 h-3.5 ${i < (request.submittedRating || 0) ? 'text-primary fill-primary' : 'text-gray-200 fill-gray-200'}`} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            ...
            <button
              ...
              disabled={approving}
              className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-primary text-white text-xs font-semibold hover:bg-accent transition-colors disabled:opacity-50"
            >
        ...
        <div className="flex items-center gap-1.5">
          {(request.status === 'PENDING' || request.status === 'REJECTED') && (
            <button
              ...
              disabled={resending}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#F4E8D8] bg-[#FFFCF8] text-[#64748b] text-xs font-medium hover:bg-[#F6F0E8] hover:text-primary transition-colors disabled:opacity-50"
            >
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-[#F4E8D8] bg-[#FFFCF8] p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-primary">Send Review Request</h3>
        ...
        <form onSubmit={handleSendRequest} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white border border-[#F4E8D8] text-sm text-primary placeholder-gray-400 outline-none focus:ring-2 focus:ring-[#14203E]/10 transition-all"
            />
          </div>
          <div className="relative">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-accent transition-colors disabled:opacity-60 whitespace-nowrap"
          >
