...
export function ContactForm({ content }: { content?: any }) {
  ...
  if (isSubmitted) {
    return (
      <div className="rounded-3xl border border-gray-100 bg-gradient-to-b from-white via-slate-50 to-white p-12 text-center shadow-xl animate-in zoom-in duration-500">
        ...
        <h3 className="text-3xl font-black text-primary mb-4 uppercase tracking-tighter">{section.successTitle || 'MESSAGE RECEIVED'}</h3>
        ...
        <Button 
          ...
          className="bg-primary hover:bg-[#F6F0E8] text-white px-8 py-6 rounded-2xl font-bold transition-all"
        >
  ...
  return (
    <div className="relative h-full overflow-hidden rounded-[2.5rem] border border-gray-100 bg-gradient-to-b from-white via-slate-50 to-white p-10 shadow-2xl">
      ...
      <h2 className="text-3xl font-black text-primary mb-8 uppercase tracking-tighter">{section.title || 'LEGAL INQUIRY FORM'}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        ...
        <Button
          type="submit"
          disabled={isLoading}
          className="w-full h-16 bg-primary hover:bg-[#F6F0E8] text-white rounded-2xl font-black uppercase tracking-widest shadow-xl transition-all disabled:opacity-50"
        >
          {isLoading ? (
