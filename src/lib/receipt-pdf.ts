...
export async function generateReceiptPdf(data: ReceiptData): Promise<Uint8Array> {
  ...
  const firmAddress = (theme?.firmAddress || brand?.firm_address || brand?.address || '').toString().trim()
  ...
  // Enterprise header
  page.drawRectangle({ x: 0, y: 764, width: 595, height: 78, color: white, borderColor: black, borderWidth: 1 })
  page.drawRectangle({ x: 40, y: 795, width: 28, height: 28, color: white, borderColor: black, borderWidth: 1 })
  text('B', 49, 803, 16, bold, black)
  text(brandText.slice(0, 58), 76, 809, 16, bold, black)
  text('PAYMENT RECEIPT', 430, 812, 12, bold, black)
  text('Professional fee receipt / client copy', 430, 796, 8, font, gray)

  y = 738
  page.drawRectangle({ x: 40, y: y - 12, width: 515, height: 42, color: lightGray })
  text(`Receipt No: ${data.number}`, 52, y + 8, 10, bold, black)
  text(`Workspace: ${workspaceName}`.slice(0, 72), 52, y - 7, 8, font, gray)
  text(`Date: ${new Date(data.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}`, 390, y + 8, 10, bold, black)
  y -= 56

  if (firmAddress) {
    text('Firm address', 40, y, 9, bold, gray)
    y -= 13
    text(firmAddress.slice(0, 100), 40, y, 8, font, gray)
    y -= 18
  }
  ...
  if (data.caseNumber || data.caseTitle) {
    ...
    if (data.caseNumber) {
      text(data.caseNumber, 80, y, 9, bold, black)
    }
    y -= 13
    if (data.caseTitle) {
  ...
  // Table header
  page.drawRectangle({ x: 40, y: y - 4, width: 515, height: 22, color: lightGray })
  text('Description', 48, y + 2, 9, bold, black)
  text('Qty', 360, y + 2, 9, bold, black)
  text('Rate', 420, y + 2, 9, bold, black)
  text('Amount', 500, y + 2, 9, bold, black)
  y -= 24
  ...
  for (const it of data.items) {
    ...
    y -= 20
    page.drawLine({ start: { x: 40, y: y + 6 }, end: { x: 555, y: y + 6 }, thickness: 0.5, color: borderGray })
  }
  ...
  y -= 22
  page.drawRectangle({ x: 360, y: y - 6, width: 195, height: 24, color: black })
  text('TOTAL', 372, y, 11, bold, white)
  text(money(data.total, data.currency), 478, y, 11, bold, white)
  y -= 30
  ...
  text(pmLabel, 140, y, 10, bold, black)
  y -= 30
    ...
    try {
      ...
      if (pmKey === 'UPI' && cfg.upiVpa) {
        ...
        if (qrPng) {
          ...
          text('Scan to pay with any UPI app', 160, y - 12, 9, bold, black)
          text(`UPI ID: ${cfg.upiVpa}`, 160, y - 28, 10, font, black)
  ...
  if (data.id && data.tenantId) {
    ...
    if (base && tenant?.slug) {
      ...
      y -= 14
      text(proofUrl.slice(0, 95), 40, y, 8, font, black)
      y -= 22
    }
  }
  ...
  page.drawRectangle({ x: 0, y: 0, width: 595, height: 46, color: white, borderColor: black, borderWidth: 1 })
  text(`Issued by: ${data.createdByName}`, 40, 62, 9, font, gray)
  text('This is a computer-generated receipt.', 40, 50, 8, font, gray)
  text('Barrister by Patience AI', 40, 18, 10, bold, black)
  text(BARRISTER_URL, 180, 18, 9, font, black)
