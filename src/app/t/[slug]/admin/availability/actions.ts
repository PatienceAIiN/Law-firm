...
export async function addSlot(slug: string, formData: FormData) {
  ...
  let day = await prisma.availabilityDay.findFirst({ where: { tenantId, date: dayDate, advocateId: advocateId || null } })
  if (!day) day = await prisma.availabilityDay.create({ data: { date: dayDate, tenantId, isActive: true, advocateId: advocateId || null } })
