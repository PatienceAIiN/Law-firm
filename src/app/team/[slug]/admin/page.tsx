...
const STAT_CARDS = (slug: string) => [
  { key: 'practice', label: 'Practice areas', icon: Briefcase, href: `/team/${slug}/admin/practice-areas` },
  { key: 'articles', label: 'Articles', icon: FileText, href: `/team/${slug}/admin/articles` },
  { key: 'lawyers', label: 'Lawyers', icon: Users, href: `/team/${slug}/admin/lawyers` },
  { key: 'cases', label: 'Cases', icon: Gavel, href: `/team/${slug}/admin/cases` },
  { key: 'inquiries', label: 'Inquiries', icon: Inbox, href: `/team/${slug}/admin/inquiries` },
  { key: 'receipts', label: 'Receipts', icon: ReceiptText, href: `/team/${slug}/admin/receipts` },
  { key: 'team', label: 'Team', icon: UserPlus, href: `/team/${slug}/admin/team` },
  { key: 'testimonials', label: 'Testimonials', icon: Quote, href: `/team/${slug}/admin/testimonials` },
  { key: 'availability', label: 'Availability', icon: CalendarClock, href: `/team/${slug}/admin/availability` },
] as const
...
export default async function TenantAdminDashboard({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!sUser?.id || sUser.tenantSlug !== tenant.slug) {
    redirect(`/team/${tenant.slug}/admin/login`)
  }
    ...
    <TenantAdminShell tenant={tenant} currentUser={currentUser}>
          ...
          return (
            ...
            >
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-primary dark:text-white">{counts[c.key] ?? 0}</p>
              </div>
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-white group-hover:bg-accent">
      ...
      <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#11151f]">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-primary dark:text-white">Your public site</h2>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              Share <code className="rounded bg-slate-100 px-1.5 py-0.5 text-[11px] dark:bg-white/10">/team/{tenant.slug}</code> with clients.
            </p>
          </div>
          <Link href={`/team/${tenant.slug}`} target="_blank" className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-xs font-semibold text-white hover:bg-accent">
