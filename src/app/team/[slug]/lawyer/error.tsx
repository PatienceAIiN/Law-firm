...
export default function LawyerError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    ...
    <div className="flex min-h-[60vh] items-center justify-center px-4">
      <div className="max-w-md rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-700 shadow-sm">
        <div className="flex items-center gap-2 font-semibold">
          <AlertTriangle className="h-5 w-5" /> Couldn't load this tab
        </div>
        <p className="mt-2 text-sm">
          {error?.message || 'A transient error occurred.'} The database is briefly over-subscribed sometimes — retry below.
        </p>
