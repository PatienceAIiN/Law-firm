...
type R = { id: string; number: string; clientName: string; clientEmail: string; total: number; currency: string; status: string; createdAt: string; caseNumber?: string | null }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-slate-500 dark:text-slate-400">{filteredReceipts.length} of {receipts.length} receipt{receipts.length === 1 ? '' : 's'} (yours)</p>
      ...
      <div className="grid gap-2 rounded-2xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-[#11151f] sm:grid-cols-[1fr_180px]">
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search receipt # or case #" className="rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-white/15 dark:bg-[#1a2030] dark:text-white" />
        <input value={date} onChange={(e) => setDate(e.target.value)} type="date" className="rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-white/15 dark:bg-[#1a2030] dark:text-white" />
      </div>
            ...
            {filteredReceipts.map((r) => (
              <li key={r.id} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-slate-500 dark:text-slate-400">{r.clientName} · {r.clientEmail || 'no email'} · {r.caseNumber ? `Case ${r.caseNumber} · ` : ''}{new Date(r.createdAt).toLocaleDateString()}</p>
                </div>
