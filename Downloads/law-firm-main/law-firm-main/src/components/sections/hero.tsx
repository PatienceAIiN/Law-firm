    ...
    <section className="bg-gray-50 px-3 py-4 sm:px-4 sm:py-6 lg:px-6 lg:py-8">
      <div className="mx-auto max-w-7xl">
        <div className="rounded-[32px] border border-gray-200 bg-gradient-to-b from-white via-[#fbfcfe] to-white p-6 text-center shadow-sm sm:p-8 lg:p-10">
          ...
          <h1 className="mx-auto mt-5 max-w-5xl text-4xl font-black uppercase tracking-tighter text-[#0a192f] sm:text-5xl lg:text-7xl">
            <span className="text-[#0a192f]">{title.split(' ').slice(0, 2).join(' ')}</span>{' '}
            <span className="text-[#c5a059]">{title.split(' ').slice(2).join(' ')}</span>
          </h1>
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Button asChild size="lg" className="h-12 rounded-2xl bg-[#0a192f] px-8 font-black uppercase tracking-widest text-white hover:bg-[#c5a059] hover:text-[#0a192f]">
              <Link href="/practice-areas">
                Explore Practice Areas
                <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="h-12 rounded-2xl border-gray-200 px-8 font-black uppercase tracking-widest text-[#0a192f] hover:bg-slate-50">
              ...
              {metrics.slice(0, 3).map((m) => {
                ...
                return (
                  <div key={m.id} className="rounded-[24px] border border-gray-200 bg-gradient-to-br from-white via-slate-50 to-white p-5 text-left shadow-sm">
                    ...
                    <div className="mt-4 text-3xl font-black tracking-tighter text-[#0a192f]">{m.value}</div>
